...
public class Collections3 {
     ...
     */
    public static Map<String, Object> extractToMap(
            final Collection<?> collection, final String keyPropertyName,
            final String valuePropertyName) {
        Map<String, Object> map = new HashMap<>(collection.size());
        try {
            for (Object obj : collection) {
                MetaObject metaObject = SystemMetaObject.forObject(obj);
                map.put(metaObject.getValue(keyPropertyName).toString(),
                        metaObject.getValue(valuePropertyName));
            }
        } catch (Exception e) {
            throw Reflections
     ...
     */
    public static <E> List<E> copyTo(List<?> source, Class<E> destinationClass) throws IllegalAccessException,
            InvocationTargetException, InstantiationException {
        if (CollectionUtils.isEmpty(source)) {
            return Collections.emptyList();
        }
        List<E> res = Lists.newArrayListWithCapacity(source.size());
        for (Object o : source) {
