...
import com.pds.p2p.core.utils.Encodes;
...
public class Servlets {
    ...
    private final static String[] STATICFILES = StringUtils.split(
            Config.get("web.staticFile"), ",");

    // 动态映射URL后缀
    private final static String UR_LSUFFIX = Config.getUrlSuffix();
    ...
    public static boolean isStaticFile(String uri) {
        if (STATICFILES == null) {
            try {
                throw new Exception(
                        "检测到“config.properties”中没有配置“web.staticFile”属性。配置示例：\n#静态文件后缀\n"
        ...
        if (StringUtils.endsWithAny(uri, STATICFILES)
                && !StringUtils.endsWithAny(uri, UR_LSUFFIX)
